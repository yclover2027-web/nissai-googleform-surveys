...
const CONFIG = {
    ...
    // 【重要】LINE Developersで発行した新しいLIFF ID（LINE専用Bot）
    LIFF_ID: '2009547264-hpAOrSp6'
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // LIFFの初期化 (LINE名自動取得・友だち追加画面は経由しない)
    initializeLiff();
 ...
 * （LINE登録経由用：友だち追加画面は経由せず、すぐにフォームを表示する）
 */
function initializeLiff() {
    const liffId = CONFIG.LIFF_ID;

    // LIFF IDが設定されていない場合はスキップ
    if (!liffId || liffId.includes('ここに')) return;
    ...
    liff.init({ liffId: liffId })
        .then(() => {
            ...
            // ※友だち確認（getFriendship）は行わず、そのまま進める
        })
        .catch(err => {
